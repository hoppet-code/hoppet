...
version 2.2.2-dev0%% VERSION-NUMBER 
%  a.k.a.\ HOPPETv1
}
\author{%
  Fr\'ed\'eric Dreyer,
...
\begin{abstract}
  This document describes \hoppet, a Fortran package for carrying out DGLAP
  ...
  %
  Unpolarised evolution
  is provided to N$^3$LO, including heavy-quark thresholds in the
  pole and $\MSbar$ schemes (the latter only to NNLO),
  %
  and longitudinally polarised evolution to NLO.
  %
  The code also includes QED evolution and N$^3$LO massless structure
  functions.
  %
  The
...
%
  ...
  evolution part of the code from F77, C/\CPP and Python.  \smallskip
\end{abstract}

\begin{textblock*}{2cm}(0.52\textwidth,0.05\textheight)  % (width)(x,y position)
...
\textbf{Note:} this manual is structured as follows: it
   consists of the original v1 manual with a few key updates, such as
   N$^3$LO evolution in section~\ref{sec:pqcd},
   %
   followed by sections~\ref{sec:v2-additions} onwards, describing the
   main additions in v2 and with updated performance studies,
   whose timings notably supersede those in section~\ref{sec:Timing}.
 }}
...
%
Note that \hoppet often uses the shorthand $t \equiv \ln Q^2$.
...
%
In general we recommend the use of negative orders, because they give
faster initialisation time.

\begin{figure}
  \centering
  \includegraphics[width=0.72\textwidth]{figs-v1/interpolation-order}%
  \caption{The different strategies (+ve and $-$ve order) for
    ...
    based.
    %
    This strategy choice affects only the interpolation as used for
    creating a representation of splitting functions.
    %
    Interpolation for evaluation of a PDF is always centred around the
    point of interest.
  }
...
be accessed indirectly through the \ttt{hoppet} module
\begin{lstlisting}
  use hoppet
\end{lstlisting}
...
have however been left out of the  \ttt{hoppet} module, in order
...
\begin{lstlisting}
  type(grid_def) :: grid
  call InitGridDef(grid,dy=0.1_dp,ymax=12.0_dp,order=-6)
\end{lstlisting}
...
\begin{lstlisting}
  ...
  call InitGridDef(subgrids(1),dy=0.30_dp, ymax=10.0_dp, order=-6)
  call InitGridDef(subgrids(2),dy=0.10_dp, ymax= 2.0_dp, order=-6)
  call InitGridDef(subgrids(3),dy=0.03333_dp, ymax= 0.6_dp, order=-6) 
...
For general usage, \hoppet provides a standard arrange of the
subgrids, which can be obtained by calling
%
the \ttt{InitGridDefDefault(grid, dy, ymax[, order])} routine.
%
That sets up the following grid
\begin{lstlisting}
...
As of version 2.1.0, it is also possible to initialise a
\ttt{grid\_def} with a class rather than a function.
%
In the \ttt{convolution} module, there is a \ttt{cong\_ignd} abstract
base class, with a procedure \ttt{f} that should be implemented in the
user's derived class:
\begin{lstlisting}
  type, extends(conv_ignd) :: my_splifn_class
    ! [...]
  contains
    procedure :: f => my_splitfn_f !< f(y,piece) function
  end type
\end{lstlisting}
The function takes two arguments: \ttt{y} as in the example above, and
also the \ttt{piece}, which is passed as an argument rather than using
the global \ttt{cc\_piece} variable.
%
This is intended to facilitate a path towards potential thread-safe
initialisation.
%
For an illustration of how one might use the class interface, users
can examine the \ttt{mvv\_splitting\_function} class in
\masterlink{src/splitting_function_interfaces.f90}, which makes it
possible to use functions with the standard MVV~\cite{NNLO-NS} interface as
\begin{lstlisting}
 call InitGridConv(grid, pgg2, mvv_splitting_function(P2GGA, P2GGB , P2GGC , 0.5_dp**3))
 call InitGridConv(grid, pqg2, mvv_splitting_function(P2QGA, null(), null(), 0.5_dp**3))
\end{lstlisting}
In this particular interface, the \ttt{A}, \ttt{B} and \ttt{C}
arguments are the regular, plus and delta-function functions, which
may be left as \ttt{null()} where the function is absent.
%
The last argument is a normalisation factor to convert from the $(\as/(4\pi))^3$
normalisation of the MVV functions to the $(\as/(2\pi))^3$
normalisation used in \hoppet.
%
It currently uses \hoppet's global $n_f$.
%
Note that as of v.2.1.0, this specific interface is not widely used
inside \hoppet and accordingly its details may evolve in future
releases.
...
and \ttt{pdf\_representation} modules, or via the \ttt{hoppet}
general module.
...
combinations, cf.\ Eq.~(\ref{eq:flavour-basis}).
%
In practice both are used inside the program and it is
...
\ttt{human} representation, such that 
...
Constants with names like
\ttt{iflv\_bbar}, \ttt{iflv\_g}, \ttt{iflv\_b}, are defined in
\ttt{module pdf\_representation}, to facilitate symbolic access to the
different flavours.
%
Note that in \CPP and Python the corresponding values are offset by
$6$, since arrays can only start with index $0$.
...
initialising a PDF:
\begin{lstlisting}
  use hoppet; use pdf_initial_condition; implicit none
  type(grid_def)    :: grid
  real(dp), pointer :: pdf(:,:)
  [...]
...
in flavour space is quite complicated. Accordingly \hoppet uses a an
``evolution'' representation of the flavour internally when carrying
out convolution of splitting matrices with PDFs, corresponding to
Eq.~(\ref{eq:flavour-basis}).
%
In the code this is referred to as the \ttt{evln} representation.
%
For most purposes the user need not be aware of this. The two
exceptions are when a user plans to create derived splitting matrices,
cf.\ Section~\ref{sec:derived-split-matrices}
(being careless about the flavour representation will lead to
mistakes), or wishes to carry out repeated convolutions for a fixed
$n_f$ value (appropriate manual changes of the flavour representation
can speed things up).
...
The internal representation of the  \ttt{evln} representation is as laid
out in Table~\ref{eq:diag_split}.
% 
When carrying out a convolution, the only non-diagonal part is the
block containing indices $0,1$.
%
Whereas the \ttt{human} representation is $n_f$-independent, the
\ttt{evln} form depends on $n_f$ through the $\Sigma$ and
$q_{\mathrm{NS}}^{V}$ entries and the fact that flavours beyond $n_f$
are left in the human representation (since they are inactive for
evolution with $n_f$ flavours).
...
Splitting function matrices and their actions on PDFs are defined in
  ...
  hoppet}). They have type \ttt{split\_mat}. Below we shall discuss
...
together with the current options for the \ntlo approximations.
%
Note that the QCD colour factors introduced in section~\ref{sec:qcd}
cannot be modified if the approximate NNLO splitting functions
...
\begin{lstlisting}
  ...
  call InitMTMNNLO(grid, MTM_NNLO)   !< MTM_NNLO is coeff of $(\as/2\pi)^2$
  call InitMTMN3LO(grid, MTM_N3LO)   !< MTM_N3LO is coeff of $(\as/2\pi)^3$
\end{lstlisting}
These set up the coefficient of $(\as/2\pi)^{2,3}$ respectively for
the convolution matrix that accounts for crossing a heavy flavour
threshold in $\MSbar$ factorisation scheme, at $\mu_F = m_h$, where
$m_h$ is the heavy-quark pole mass, as has been described in
section~\ref{sec:pqcd}. Since the corresponding NLO term is zero, the
number of flavours in $\as$ is immaterial up to and including \ntlo.
%
The \ttt{mass\_threshold\_mat} objects are composed of four
pieces\footnote{This structure has been substantially revised as of
  \hoppet v2.2.0, and was introduced to allow for the general flavour
  structure that can appear at any order. } 
\begin{lstlisting}
...
\end{lstlisting}
This provides the complete flavour structure at any order of
perturbation theory.
%
As with normal splitting functions, at lower orders some terms are
either equal to others or zero (e.g.\ NShV is zero at NNLO).

A different \ttt{mass\_threshold\_mat} is needed for each different
number of flavours, with the valid number of light flavours accessible
as \ttt{P\_light\%nf\_int}.
%
With an \ttt{MTM\_NNLO} object where that number of light flavours is
equal to 3 then to take a PDF in the effective theory with $n_f=3$
active flavours \ttt{pdf\_nf3}, and convert it to the corresponding
PDF in the effective theory with $n_f=4$ active flavours
\ttt{pdf\_nf4} at $m_h^2$ (pole mass), one uses code analogous to the
following
\begin{lstlisting}
  real(dp) :: pdf_nf3(:,:), pdf_nf4(:,:)
  [ ... ]
  pdf_nf4 = pdf_nf3 + (as2pi)**2 * (MTM_NNLO * pdf_nf3)
\end{lstlisting}
...
Labelling \ttt{nl} and \ttt{nh} as the number of light and light+heavy
flavours respectively, the following further operations are valid
\begin{lstlisting}
  type(split_mat)          :: sm_nl   !< split_mat for nf=nl flavours
  type(split_mat)          :: sm_nh   !< split_mat for nf=nh=nl+1 flavours
  type(mass_threshold_mat) :: mtm     !< a pre-existing mass_threshold_mat (nl->nh)
  type(mass_threshold_mat) :: new_mtm !< a new mass_threshold_mat to be created
  [ ... ]
  
  ! creation
  call InitMTM(grid, mtm_new, nl)    !< allocates and sets mtm_new to zero
  call InitMTM(mtm_new, mtm)         !< copies mtm
  call InitMTM(mtm_new, sm_nh)       !< mtm_new*q (nl->nh) gives the same result as sm_nh*q

  ! addition (with optional factor multiplying the second argument)
  call AddWithCoeff(mtm_new, mtm  , [factor])  !< mtm_new += mtm[*factor]
  call AddWithCoeff(mtm_new, sm_nl, [factor])  !< adds just to P_light
  call AddWithCoeff(mtm_new, sm_nh, [factor])  !< converts sm_nh to an mtm and adds it

  ! multiplication / convolution
  call Multiply(mtm_new, fact)
  call SetToConvolution(mtm_new, mtm, sm_nl)   !< mtm_new = mtm * sm_nl
  call SetToConvolution(mtm_new, sm_nh, mtm)   !< mtm_new = sm_nh * mtm

  ! deallocation of associated memory
  call Delete(mtm_new)
\end{lstlisting}
The \ttt{InitMTM} and \ttt{SetToConvolution} operations allocate
memory if \ttt{mtm\_new} has not been allocated, otherwise they reuse
the existing memory, requiring a consistent underlying grid.
%
The above operations effectively provide the full valid algebra
associated with mass thresholds for a single additional heavy flavour.

In addition, \hoppet allows the user to create ``derived''
\ttt{mass\_threshold\_mat} objects, in analogy with the derived
\ttt{split\_mat} objects discussed in
Section~\ref{sec:derived-split-matrices}.
%
The relevant calls are 
\begin{lstlisting}
  integer                  :: nl            !< number of light flavours
  real(dp), pointer        :: probes(:,:,:) !< the temporary probes
  type(mass_threshold_mat) :: mtm           !< output mass_threshold_mat

  call GetDerivedMTMProbes(grid, nl, probes)
  [... operate on each of the probes as if it were a normal PDF ]
  call InitMTM(grid, mtm, nl)
  call SetDerivedMTM(mtm, probes)  !< mtm*q now reproduces operations carried out on probes
\end{lstlisting}
%
% The \ttt{P\_light} splitting matrix is specific to a given number of
% light flavours (\ttt{P\_light\%nf\_int}) and so a different \ttt{mass\_threshold\_mat}
% 
...
As for the NNLO splitting functions, the NNLO mass threshold corrections
...
\begin{lstlisting}
  integer :: nnlo_threshold_variant
  call dglap_set_nnlo_nfthreshold(nnlo_threshold_choice)
\end{lstlisting}
with the following choices defined (as integer parameters), again in
...
At \ntlo,  the
single-mass thresholds have been calculated in
Refs.~\cite{Bierenbaum:2009mv,Ablinger:2010ty,Kawamura:2012cr,Blumlein:2012vq,ABLINGER2014263,Ablinger:2014nga,Ablinger:2014vwa,Behring:2014eya,Ablinger:2019etw,Behring:2021asx,Ablinger:2023ahe,Ablinger:2024xtt,Behring:2025avs}.
%
They are available in two forms,
using code from Refs.~\cite{BlumleinCode,Fael:2022miw}.
%
One can choose which form to use by calling
\ttt{dglap\_set\_n3lo\_nfthreshold(n3lo\_threshold\_choice)},
with one of the following values for \ttt{n3lo\_threshold\_choice}:
\begin{itemize}
\item \ttt{n3lo\_nfthreshold\_libOME} (default):
  %
  ...
  %
  This is the default choice.\footnote{Users wishing to investigate
    the effect of the \ttt{NShV} piece calculated in
    Ref.~\cite{Behring:2025avs} can turn it off by selecting
    \ttt{n3lo\_nfthreshold\_libOME\_2510}.}
...
\begin{lstlisting}
  type dglap_holder
    ...
    type(mass_threshold_mat), pointer :: allMTM(3:nloop,nflo+1:nfhi)
    type(mass_threshold_mat), pointer :: MTM_NNLO, MTM_N3LO ! pointers to MTM(3,nf) and MTM(4,nf)
    logical                  :: MTM2_exists
...
\ttt{factscheme} and \ttt{nloop}.
%
Other parts are set up once and for
all on initialisation, notably the \ttt{allP} array, which contains
the 1-loop, 2-loop, etc. splitting matrices for the requested $n_f$
range, and, analogously the \ttt{allMTM} array, whose flavour index
corresponds to the number of \emph{heavy} flavours.

Pointers such as \ttt{P\_LO} are shorthands for the objects at the
current number of flavours.
%
...
\end{lstlisting}
for convenient named access to the various splitting matrices, and
similarly for the mass threshold matrices (with \ttt{nf\_lcl}
corresponding to the number of flavours including the heavy quark).
...
\ttt{pdf\_tabulate} module, or accessible also from \ttt{hoppet}.
...
F77. An include file \ttt{src/hoppet.h} is provided for calling
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{figs-v1/grid}%
...
\begin{figure}
  \centering
  \includegraphics[width=0.48\textwidth]{figs-v1/acc-dy0.2-dlnlnQ0.05.pdf}\hfill
  \includegraphics[width=0.48\textwidth]{figs-v1/acc-dy0.05-dlnlnQ0.0125.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=0.48\textwidth]{figs-v1/dy}%
  \hfill
  \includegraphics[width=0.495\textwidth]{figs-v1/dlnlnQ}%
...
\noindent \fbox{\parbox{\textwidth}{%
    NOTE: the timings shown here date from 2008 and have been
    kept for historical reasons.
    %
    Updated timings as of 2025 are to be found in
    Section~\ref{sec:v2-performance}.
}}\medskip

The time spent in \hoppet for a given analysis can be expressed as
...
in the case with pre-prepared cached evolution, $t_p$ is the time for
...
\begin{table}
 ...
 for    $\dy=0.2$ and 
    ...
    %
    These are timings from 2008, see Section~\ref{sec:v2-performance}
    for 2025 timings.
  }
  \label{tab:timings}
\end{table}
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{figs-v1/acc-v-time}%
  \caption{relative accuracy obtained as a function of the time taken
    ...
    cache).
    %
    The timings are from 2008. See Section~\ref{sec:v2-performance}
    for 2025 timings.
  }
...
\section{Additions in version 2}
\label{sec:v2-additions}
The following sections detail additions in \hoppet{} that appeared in
v2.0.0. 
...
\ttt{examples/f90/tabulation\_example.f90}. Compilation instructions
...
\ttt{examples/f77/tabulation\_example.f} (cf.\ section~\ref{sec:stream-example}).
...
contains 
  ...
  function unpolarized_dummy_pdf(xvals) result(pdf)
    ...
    ! labels iflv_g, etc., come from the hoppet module
...
\noindent The file
\ttt{examples/f90/tabulation\_example.default\_output} contains a copy
...
found in
\ttt{examples/f90/tabulation\_example\_streamlined.f90}. 
A program with same interface and same output 
but in F77, is to be found in
\ttt{examples/f77/tabulation\_example.f}.
...
\subsection{Interface example with QED included}
  ...
  It can be found in \ttt{examples/f90/tabulation\_example\_qed.f90}.
...
additional examples are available in the \ttt{examples/f90/} and
\ttt{benchmarking/} directories. One,
\ttt{examples/f90/tabulation\_example\_2.f90}, summarised briefly
...
\ttt{examples/f90/sumrules.f90}, also illustrates the use of
...
which is available in the \ttt{examples/f90} 
...
%accessible through the common {\tt hoppet} module. For
...
\begin{table}
 ...
 \\
 \vspace{0.2cm}
  \input{interface-tables/streamlined.tex}
...
At \ntlo~\cite{Gracey:1994nn,Davies:2016jie,Moch:2017uml,Falcioni:2023luc,Falcioni:2023vqq,Moch:2023tdj,Falcioni:2024xyt,Falcioni:2024qpd,Falcioni:2025hfz}, the user can call
\begin{lstlisting}
  integer :: n3lo_splitting_approximation, n3lo_splitting_variant
  call dglap_Set_n3lo_approximation(n3lo_splitting_variant)
  call dglap_Set_n3lo_variant(n3lo_splitting_variant)
\end{lstlisting}
with one of the following options for the approximation
\begin{itemize}
\item \ttt{n3lo\_splitting\_approximation\_up\_to\_2310}, with the
  approximations in papers up to and including Ref.~\cite{Moch:2023tdj};
\item \ttt{n3lo\_splitting\_approximation\_up\_to\_2404}, with the
  approximations in papers up to and including
  Ref.~\cite{Falcioni:2024xyt};
\item \ttt{n3lo\_splitting\_approximation\_up\_to\_2410}, with the approximations in papers up to and including
  Ref.~\cite{Falcioni:2024qpd}.
\item \ttt{n3lo\_splitting\_approximation\_up\_to\_2512}, the default value at the time of
  writing (v2.2.0), with the approximations in papers up to and including
  Ref.~\cite{Falcioni:2025hfz},
\end{itemize}
and one of the following three for the variant
\begin{lstlisting}
  nnlo_splitting_Nfitav     !< default
  nnlo_splitting_Nfiterr1
  nnlo_splitting_Nfiterr2
\end{lstlisting}
...
\texttt{hoppet} module), which specifies the \texttt{kind} of real
