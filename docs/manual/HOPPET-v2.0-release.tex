...
%
The streamlined interface has long been available from Fortran and
\CPP and, as of v2, can be accessed also in Python.
%
The core methods of the streamlined interface are illustrated in
Table~\ref{tab:streamlined-interface}. 


From modern Fortran, one has access to the full code functionality,
the ``general'' interface, including low-level objects that are useful
in DGLAP evolution, such as splitting functions, splitting matrices,
tabulations of PDFs, etc.

\begin{table}[t]
  \centering
  \input{interface-tables/streamlined.tex}
  \caption{Core methods of the streamlined interface.
    %
    In Python, \ttt{hoppetStart(...)} is to be replaced with
    \ttt{hoppet.Start(...)}, and \ttt{hoppet.Eval(...)} returns
