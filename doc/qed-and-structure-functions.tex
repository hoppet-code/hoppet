...
$L$ is the \commentpn{Comment by Gavin here, can't understand it} logarithm of a \commentpn{a $\to$ the}
...
\gps{Giulia had found a ref that gave a nice explanation, maybe we can
  add that ref here?}
\commentpn{What about: In fact, at that order, there are also graphs with three
  electromagnetic vertices on the quark line and three on the lepton line, that change sign if the lepton line is charge-conjugated.}
%
...
\commentpn{Actually this is also true in a fixed flavour number scheme,
  as long as mass effects are not being included in the coefficient functions,
  i.e. for $Q\gg m$ variable flavour is the same as fixed flavours
  (including the flavour of mass $m$ as a light flavour.)}
...
%
Specifically \commentpn{the} (integer) \ttt{scale\_choice}
...
\item \ttt{scale\_choice\_arbitrary} allows the user to choose
  ...
  separated by perturbative order \commentpn{I would prefer:
    as arrays, one for each perturbative order,} and with dedicated additional arrays
