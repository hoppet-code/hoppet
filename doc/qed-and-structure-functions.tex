...
N$^3$LO evolution. We also present updated plots and
tables\ak{Currently no table. Do we want to redo table 2?} at NNLO, to
give a better idea of the performance of \hoppet{} on typical
compilers and hardware in 2025.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth,page=1]{hoppet-v2-accuracy-speed-v-dy.pdf}
        \caption{}
        \label{fig:sub1}
    \end{subfigure}
    \hfill
    ...
    \caption{Accuracy (top) and timing (bottom) versus \texttt{dy} at
      ...
      \texttt{-O3} optimisation \ak{with some average over N
        runs?}. We have further used \texttt{ymax = 12} and
      \texttt{dlnlnQ = dy/4}.}
    \label{fig:main}
\end{figure}
...
functions and exact mass thresholds \ak{GS confirm. Do we need to
  specify what we do at n3lo since we don't really have a choice?}. To
...
flavours (guds). The full tabulation covers the range $10^{-5} < x <
1$, $2 < Q < 10^4\, \mathrm{GeV}$ but since the grids quickly
...
respectively. At NNLO the accuracy comes out similar to previous
versions of \hoppet{}. In particular we note that with \texttt{dy =
  0.2} one obtains a relative accuracy of $10^{-4}$ across all
flavours in the range $x < 0.9$. At the finest grid spacing \texttt{dy
  = 0.04} a relative accuracy of $10^{-7}$ can be achieved, good
enough for precise benchmark comparisons as were for instance carried
out in Refs.~\cite{Dittmar:2005ed,Bertone:2024dpm}\ak{Do we want to
  mention that it is also much better than the most current n3lo
  benchmark which barely reaches $10^{-4}$?}. At N$^3$LO we observe a
similar level of accuracy as at NNLO except for the all-flav, $x<0.9$
case with \texttt{dy} below \texttt{0.1}. We have confirmed that this
deterioration is due to small numerical instabilities in the exact
mass threshold expressions at N$^3$LO, presumably when $x \to 1$. We
therefore suspect that this could be improved by switching to a
large-$x$ expansion above some threshold. Since this deterioration is
only present at the relative $10^{-5}$-level, and at very large values
of $x$, it is unlikely to be visible in any realistic application. In
particular we note that for $x<0.7$ the accuracy is comparable to that
at NNLO.

For the timing studies we run \hoppet{} for different values of
\texttt{dy} on an \texttt{M2Pro} with \texttt{gfortran v14.2} and
\texttt{-O3} optimisation. In order to avoid fluctuations we take the
average over \ak{N} runs.

\ak{taken from v1 manual with small modifications} The time spent in
\hoppet for a given analysis can be expressed as follows, according to
whether or not one carries out cached evolution (pre-evolution) or
not:
\begin{subequations}
  \label{eq:timing-v2}
  \begin{align}
...
Here we study $t_s$, $t_i$, and $t_c$. The results can be seen in the
bottom panels of Figures~\ref{fig:sub1}--\ref{fig:sub2} at NNLO and
N$^3$LO respectively.
%
The timings are seen to obey a power law in \texttt{dy}. \ak{might be
  of interest to extract the coefficients}.\gps{I would expect
  $1/\text{\texttt{dy}}^3$ for the evolution and the plot seems
  roughly consistent with that; in fact perhaps we want to quote the
  expected timing as $(\text{dy}^2 \text{dlnlnQ})^{-1}$.
  %
  I think that we expect precision to go as $\text{dlnlnQ}^4$, while
  the $\text{dy}$ order might be higher.
}
%
At
NNLO the initialisation time of $\sim 60-300\,\mathrm{ms}$ dominates
the evolution time across almost all \texttt{dy} values that we
study. We note that when using cached evolution the evolution time
reaches as little as $1\,\mathrm{ms}$ for \texttt{dy = 0.2}, making
the code fast enough to perform accurate PDF fits in a few minutes
even when $n_i$ reaches $10^5$.
\gps{Do we have a basis for this number for $n_i$, i.e.\ something we
  can cite?}
%
...
\fi
%
which were obtained with 2008 hardware, we see a speed-up of roughly a
factor 10.
...
initialisation is insignificant. Should parametrised expressions of
the mass threshold functions become available we plan to implement
them to overcome this minor inconvenience \ak{GS rephrase}.
...
\label{sec:lhapdf}
A minor new feature of this release is the added possibility to save a
\hoppet{} table in the LHAPDF6~\cite{LHAPDF} grid format. Access to
this functionality is also provided in the streamlined interface. The
routine has the following structure
...
needed by LHAPDF. If \ttt{pdf\_index} is equal to 0 then the routine
will output the content of the table in \ttt{basename\_0000.dat} and
write a template \ttt{basename.info} again in the LHAPDF
format. \hoppet{} will fill most of the entries in the \ttt{.info}
file, but a few need to be filled manually by the user. For any other
value of \ttt{pdf\_index} only the corresponding \ttt{.dat} file will
be written.
...
flavours of the PDF, even if they are zero. The user can overwrite
this default behaviour by providing an array of flavours and their pdg
values. If \ttt{iy\_increment > 1} a coarser grid is provided by
...
an extra factor half. The \ttt{flav\_rescale} array only needs to be
...
interface through \CPP{} or Python. In this case the routine is
...
\end{lstlisting}
The routine writes the content of the streamlined interface
