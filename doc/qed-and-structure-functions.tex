...
\begin{itemize}
 ...
 \item
   ...
   same as above but the example shows how to efficiently read in all
   LHAPDF members, and computes the PDF uncertainty,
 \item \masterlink{example_cpp/with-lhapdf/lhapdf_to_hoppet.cc}, which
   includes a routine called \ttt{void load\_lhapdf\_assign\_hoppet(const
     string \& pdfname, int imem=0)} which can be directly copied over to a
   user's application directly,
 \item \masterlink{example_py/lhapdf_to_hoppet.py}, makes use
   of \ttt{hoppet.lhapdf.load()} which can be
   made accessible with \ttt{from hoppet import lhapdf}.
\end{itemize}
%
...
\begin{table}[htbp]
  ...
  %%hoppet-v1.2                  & 313     &        &
    ...
    % \multicolumn{2}{|c|}{\rule{0pt}{2.5ex}LHAPDF 6.5.5 + patch}
    %                            &114$^\dagger$ &  87    & 1645$^\dagger$ \\
