...
\gps{I found the next section quite hard reading, especially coming so
  early; options include moving the whole QED section later, or
  putting the streamlined part first, with the \ttt{iflv\_photon}
  specified there etc.}
...
%
\gps{Reading this it's not very clear what to do. We should be
  explicit, e.g.\ set a variable in a given module. Similarly
  below. Perhaps we also want to put an itemize?}
%
...
available in the future we have also implemented a flag,\gps{flag=boolean?}
...
The structure functions can be accessed by using 
...
the indices \gps{are the names obvious enough?}
\begin{lstlisting}
...
\end{lstlisting}
with \ttt{order\_max} \gps{is that not \ttt{order\_max}$-1$?}
%
the maximal
...
\end{figure}
%
It gives the $F_2^{\rm NC}$ \gps{corresponds to index iF2EM. Confusing?} structure function at various
...
argument\gps{rephrased}.
%
Instead in Python, those routines return a \gps{sometimes numpy?}
Python array directly.
%
...
initial conditions. \gps{tried to be clearer in what comes next}
%
...
\ttt{x} and
\ttt{Q} that returns a \gps{numpy?} Python array of flavours (see the
...
from the main directory \gps{This or preferred \ttt{cmake --build
    build} etc?}\gps{also added ctest}
\begin{lstlisting}
...
and Python interfaces.\gps{update if we change Python?}
%
...
Of particular note to most users are \gps{check Python on/off}
\begin{lstlisting}
...
%
\gps{added:}
However the penalty can be noticeable in interactive work. \gps{Say we
  look forward to forthcoming faster options?}
%
...
the interpolation order below $4$.\gps{gain$\to$loss}
%
...
\begin{table}[htbp]
  ...
  \\
  %
  $^\dagger\,$Note that in its Fortran (\ttt{EvolvePDF(...)}) and Python (\ttt{pdf.xfxQ(...)})
  interfaces, \gps{have we checked Python?} this version of
