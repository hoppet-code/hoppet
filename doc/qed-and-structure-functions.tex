...
be imported into a \ttt{Python} instance through \ttt{import
  hoppet}. For a simple tabulation example the user should take a look
at
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth,page=2]{figs/hoppet-v2-accuracy-speed-v-dy.pdf}
        \caption{}
        \label{fig:main-sub1}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth,page=4]{figs/hoppet-v2-accuracy-speed-v-dy.pdf}
        \caption{}
        \label{fig:main-sub2}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth,page=3]{figs/hoppet-v2-accuracy-speed-v-dy.pdf}
        \caption{}
        \label{fig:interp-orders}
    \end{subfigure}
    \caption{
      ...
      %
      The timing corresponds to the \ttt{EvalPdfTable\_yQ(table,y,Q)} modern
      Fortran call.
      %
      Calling \ttt{EvalPdfTable\_xQ(table,x,Q)}, or the corresponding
...
\end{lstlisting}
to set the interpolation order in $y$ and $\ln\ln Q$.
%
These calls globally override any table-specific interpolation order
settings.
%
A value of $2$ for both of them corresponds to quadratic
interpolation, while a value of $3$ corresponds to cubic
interpolation.
%
...
%
These rather high interpolation orders are to guarantee numerical
accuracy in a normal \hoppet{} run, but come with a speed penalty
because of the larger number of operations.
%
However, if PDF evaluation represents a significant fraction of the
time for a user's code, they can choose to lower the interpolation
order and reduce \ttt{dy} and \ttt{dlnlnQ} so as to retain good accuracy.
...
\begin{table}[htbp]
  ...
  \ttt{hoppetEval} with all flavours, and the equivalent call in LHAPDF.  \ak{Gavin to update with M2Pro}}
...
For users who wish to use an existing LHAPDF grid directly with
\hoppet{}, we provide examples in Fortran, CPP{}, and Python, which
can be found here \\[0.2cm]
\repolink{example_f90/with-lhapdf/fast_pdf_evaluation.f90}{example\_f90/with-lhapdf/fast\_pdf\_evaluation.f90}, \\
\repolink{example_cpp/with-lhapdf/cpp_fast_pdf_evaluation.cc}{example\_cpp/with-lhapdf/cpp\_fast\_pdf\_evaluation.cc}, \\
\repolink{example_py/tabulation_example_lhapdf.py}{example\_py/tabulation\_example\_lhapdf.py}. \\[0.2cm]

The Fortran and \CPP{} examples are virtually identical, and the
\ttt{subroutine} respectively \ttt{void}
\ttt{load\_lhapdf\_assign\_hoppet} can in principle be used in
...
through \ttt{hoppetAlphaS}. All three examples will also print the
