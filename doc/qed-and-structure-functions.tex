...
N$^3$LO respectively. The timings are seen to obey a power law in
\texttt{dy} \ak{might be of interest to extract the coefficients}. At
...
even when $n_i$ reaches $10^5$. Comparing these numbers to those of  
%
\ifreleasenote
...
but the initialisation times are dramatically different in the range
$4-15\, \mathrm{s}$. This boils down to the extra time that it takes
to initialise the exact mass threshold functions of
Ref.~\cite{BlumleinCode}. In any application where \hoppet{} either
...
through most package managers (e.g.\ \texttt{apt install swig}) but
