...
\begin{figure}[htbp]
    ...
    \caption{Accuracy (top) and timing (bottom) versus \texttt{dy} at
      ...
      \texttt{dlnlnQ = dy/4}.
      %
      \gps{Should second plot move to \S~\ref{sec:fastpdf}? And put
        $Y$-order first, as in the override call?}  \gps{Comment on
        \texttt{eps} as concerns initialisation time, or cut off the
        plot} }
    \label{fig:main}
\end{figure}
...
In the streamlined interface the user can now call
\begin{lstlisting}
...
order is quartic in the \ttt{lnlnQ} direction, while \hoppet{}
dynamically picks an order in \ttt{y} between $6$ and $10$
(corresponding to \ttt{yorder = -1}). These rather high interpolation
