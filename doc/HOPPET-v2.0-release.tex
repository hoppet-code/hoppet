...
\begin{abstract}
  ...
  we describe a new Python interface, CMake build option, \ak{and
    functionality to save a \hoppet{} table as an LHAPDF grid. This
    release breaks backwards compatibility insofar only that the
    suffix \texttt{v1} has been dropped from \texttt{hoppet\_v1} in
    all places -- most importantly from \texttt{libhoppet\_v1} which
    has now become \texttt{libhoppet} and \texttt{module hoppet\_v1}
    which has become \texttt{module hoppet}.}
\end{abstract}
