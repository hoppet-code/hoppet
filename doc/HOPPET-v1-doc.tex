...
The joint QED+QCD evolution implemented in \hoppet is as described in
Refs.~\cite{Photon+Lepton-PDFs}. \comment{Fix Ref!!!}
%
In deciding which contributions should be included, those references
take a consistent set of set within a ``phenomenological'' counting.
%
In that counting, one considers $\alpha\sim \as^2$, and takes the
...
considered to be of order $L \sim 1/\as$.
%
From that point of view, NNLO (3-loop) QCD evolution brings control of
terms up to $\as^{n+2} L^n \sim \as^2$.
%
To achieve a corresponding accuracy when including QED contributions,
\hoppet can account for
\begin{itemize}
\item 1-loop QED splitting functions, which first contribute at order
  $\alpha L \sim \as$, i.e.\ like NLO in QCD
  
\item 2-loop mixed QCD-QED splitting functions~\cite{}, which first contribute
  at order $\alpha \as L \sim \as^2$, i.e.\ like NNLO QCD.

\item 1-loop QED running coupling, including lepton and quark
  thresholds, which first contributes at order $\alpha^2 L^2 \sim
  \as^2$, like NNLO QCD.
  
\item optionally, the 2-loop pure QED $P_{\ell q}$ splitting function~\cite{},
  which brings absolute accuracy $\alpha^2 L\sim \as^3$ to the lepton
  distribution (which starts at $\alpha^2 L^2 \sim \as^2$).
  %
...
%
The above counting is to be contrasted with a ``democratic'' scheme
where one would consider $\alpha \sim \as \sim 1/L$ and one would aim
to take the same loop order across all couplings and splitting
functions, regardless of the relative numbers of QCD and QED couplings
that they involve.
...
An example involving QED+QCD evolution is to be found in
...
\end{lstlisting}
By default it is initialised with the PDG~\cite{PDG2022} value for the
...
accuracy.
%
Lepton masses are hard-coded to their PDG values~\cite{PDG2022}, as
coded in the \ttt{src/qed\_coupling.f90} file.
...
\ttt{iflv_xxx} index values are available:
\begin{lstlisting}
