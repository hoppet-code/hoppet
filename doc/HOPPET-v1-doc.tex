...
\abstract{This document describes a Fortran~95 package for carrying
  ...
  functions and PDFs, making it possible for a user to extend the
...
initial condition, $n_{xQ}$ is the number of points in $x,Q$ that one
evaluates per PDF initial condition; in the case with preprepared
cached evolution, $t_p$ is the time for a preparing a cached evolution
and $t_c$ is the time for performing the cached evolution.


\begin{table}
  \centering
  \begin{tabular}{|ll|c|c|c|c|}\hline
          &&  Lahey   &  Intel   & g95   & gfortran \\\hline
   $t_s$ &[s]  &  0.9 &          &       &          \\
   $t_\alpha$ &  
               &      &          &       &          \\
   $t_i$ &[ms] &  37  &          &       &          \\
...
\begin{table}
  ...
  \begin{tabular}{|l|c|c|l|} \hline
    Parameter & Default & Affects & Notes \\ \hline
    \ttt{dy}  &  $-$    &   $t_s, t_i, t_p, t_c$    & also other $y$ grid params \\
    ...
    \ttt{dlnlnQ}$\\
    \ttt{dlnlnQ}       &  $[\ttt{dy}/4] $ &  $t_i, t_p, t_c$ & should be $\sim
    \ttt{dy}/4$ \\
    \ttt{lnlnQ\_order} &  $4$ &  $t_{xQ}$ &  \\ \hline
    \ttt{DefaultCouplingDt} & $0.2$ & $t_\alpha$ & sufficient for $\epsilon < 10^{-9}$\\
    \hline
  \end{tabular}
