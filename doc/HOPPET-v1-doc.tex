...
\item optionally, the 2-loop pure QED $P_{\ell q}$ splitting
  ...
  NLO accuracy as the photon distribution, then it should be
  included.
\end{enumerate}
%
...
\begin{lstlisting}
    subroutine InitQEDCoupling(coupling_qed, effective_light_quark_masses, &
...
By default it initialises the parameters relevant to the QED coupling
and its running.  The electromagnetic coupling at scale zero is set by
default to its PDG~\cite{ParticleDataGroup:2022pth} value, unless the
optional argument \ttt{value\_at\_scale\_0} is provided, in which case
the latter is taken as the value of the QED coupling at zero momentum.

The running is performed at leading order level using seven thresholds:
a common effective mass for the three light quarks, the three lepton
masses, and the three masses of the heavy quarks.
%
...
active matter content changes. This information is then used to set
the arrary \ttt{nflav(3,0:n\_thresholds)}, where \ttt{nflav(1:3,i)}
contains an integer array with the number of leptons, down and up
quarks at a given $Q^2$ such that \ttt{threshold(i-1)}$<Q^2<$
\ttt{threshold(i)}. \ttt{threshold(1:7)} are active thresholds, while
...
The function \ttt{Value(qed\_coupling,mu)} returns the QED coupling at
scale $\mu$.

\paragraph{QED splitting matrices}
...
\end{lstlisting}
which contains the LO, NLO and NNLO splitting matrices 
\begin{lstlisting}
...
respectively. Besides \ttt{nf}, these splitting matrices also need the
number of up-type quarks (\ttt{nu}), down-type quarks (\ttt{nd}), and
leptons (\ttt{nl}).
%
Note that the splitting functions of order $\alpha$ (i.e.\ \ttt{01})
for the leptons are not given since they can be obtained from the ones
...
\end{lstlisting}     
initializes all \ttt{qed\_split\_mat} objects and sets all QED
...
%\end{lstlisting}
%which sets \ttt{nl,nu,nd,nf} for \ttt{qed\_split\_mat\_lo},
%\ttt{qed\_split\_mat\_nnlo} and \ttt{qed\_split\_mat\_nnlo}.
\commentpn{It is unclear to us how crossing the flavour trheshold in
  evolution are handled. The InitQEDSplitMat seems to work for fixed
  number of flavours ...}
...
QED splitting functions can be represented by \ttt{.conv.} operator or
...
%
The two indices of the \ttt{pdf} refer to
the number of $x$ values in the \ttt{grid}, and the flavour.
The flavour indices for photons and leptons in the PDF arrays are
given by
\begin{lstlisting}  
...
\end{lstlisting}
where for the leptons the sum of lepton and anti-lepton (which are
identical) is always given.
...
Next we must prepair a \ttt{pdf\_table} object forming the interpoleting grid
for the evolved PDF's. This is initialized by a call to the
subroutine
\begin{lstlisting}
  subroutine AllocPdfTableWithLeptons(grid, pdftable, Qmin=1.0_dp, Qmax=10000.0_dp, & 
       & dlnlnQ = dy/4.0_dp, freeze_at_Qmin=.true.)
\end{lstlisting}
that is identical to the one without leptons, the only difference is that the
maximum pdf flavour index now includes the leptons.

The full evolution for filling the table is performed by calling the
subroutine
...
\begin{lstlisting}
    ...
    &  coupling, coupling_qed, nloop_qcd, nloopqcd_qed)
     [...] 
\end{lstlisting}
where \ttt{table} is the outpdf, \ttt{Q0} the initial scale and
\ttt{pdf0} is the PDF at the initial scale. Note that, by default,
only the leading QED evolution is included. If one wishes to include
also mixed QCD-QED corrections one needs to set
\begin{lstlisting}
    nqcdloop_qed = 1
\end{lstlisting}
before this call. Similarly, setting
\begin{lstlisting}
    with_Plp_nnloqed=.true.
\end{lstlisting}
includes also the NNLO $P_{lq}$ splittings in the evolution. 
...
% Even within the phenomenological counting, once N3LO QCD splitting
% functions~\cite{N3LOSplitFn} are fully computed and included in
% \hoppet, it will make sense to include additional QCD and mixed
% QED-QCD contributions.
...
\begin{lstlisting}
    ...
    call InitQEDCoupling(coupling_qed, effective_light_quark_masses, &
                         quark_masses(4:6) [,value_at_scale_0])
\end{lstlisting}
By default it initialises the QED coupling to the
PDG~\cite{ParticleDataGroup:2022pth} value for the electromagnetic
coupling at scale zero and evolves it using the 1-loop QED evolution
up to high scales.
%
...
anti-lepton distributions are identical. Note that if one extends the
...
between top and anti-top quarks at the LHC~\cite{Nason:1989zy}.
\commentpn{Is this really right? an asymmetry can only arise from a
  \ttt{Plq\_03} splitting function ...}
%
%\comment{Does this persist at higher orders or not?}
%
\footnote{Were one to use \hoppet to obtain the distribution of
  ...
  %
  See for example discussion in Ref.~\cite{Frixione:2023gmf}\commentgz{updates refs}.
}


The initializaton of the grid to perform the evolution and of the
$\ttt{dglap\_holder}$, which stores all the components required for the
evolution proceeds as in the pure QCD case.
This starts by calling the \ttt{InitGridDefDefault}
and \ttt{InitDglapHolder}.
Furthermore one needs to
initialize the QCD splitting matrices using 
\begin{lstlisting}
  call InitQEDSplitMat(grid, qed_split)
\end{lstlisting}
and one allocates the table of PDFs including the leptons
\begin{lstlisting}
...
\begin{lstlisting}
  call AddNfInfoToPdfTable(table,coupling)
\end{lstlisting}
Finally one creates the tabulation based on the evolution of the PDF from the starting scale $Q_0$ including QED corrections of items 1. and 2. listed in Sec.~\ref{sec:qed-evolution}
\begin{lstlisting}
  call EvolvePdfTableQED(table, Q0, pdf0, dh, qed_split, &
                         coupling, coupling_qed, nloop_qcd, nloopqcd_qed)
\end{lstlisting}
When the argument ${\tt nloopqcd\_qed}$ is set to 1 also the mixed QCD $\times$ QED corrections of item 3. are included. Furthermore, if one sets 
\begin{lstlisting}
with_Plp_nnloqed=.true.
\end{lstlisting}
then also the $P_{lq}$ correction of item 4. is included.\commentgz{Note: at the moment with this flag set to true the $P_{lq}$ contribution is included regardless of the value of $\tt{nloopqcd\_qed}$. Perhaps one should change this?} 
...
The streamlined interface including QED effects works as in the case of pure QCD evolution. Six routines have been added:
\begin{lstlisting}
  call with_qed_true (or call with_qed_false)
  call with_qcd_qed_true (or call with_qcd_qed_false) 
  call with_Plq_true (or call with_Plq_true) 
\end{lstlisting}
The first one turns on/off QED evolution at order ${\cal O}(\alpha)$
(i.e. items 1. and 2. in Sec.~\ref{sec:qed-evolution}). The second
turns on/off mixed QCD$\times$QED effects in the evolution (i.e. item
3. in Sec.~\ref{sec:qed-evolution}) and the last turns on/off the
order $\alpha^2$ $P_{\ell q}$ splitting function (i.e. item 4. in
Sec.~\ref{sec:qed-evolution}). By default all QED corrections are off. 
