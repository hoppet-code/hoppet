...
\begin{lstlisting}
  ...
  ! PDF set in human representation -> Sum over all pdfs
  ...
  moment_index=1_dp 
  momentum_sum_rule = GetTruncMoment(grid,pdf_flav,moment_index)
\end{lstlisting}
An example of the computation of sum rules can be found
in {\tt example\_f90/sumrules.f90}.


The convention for the definition of truncated moments
as implemented in {\tt GetTruncMoment}
in \hoppet is given by
\be
M_N\lc xq(x,Q^2)\rc \equiv \int_0^{y_{\rm max}} dy e^{-yN} xq\lp y,Q^2 \rp \ ,
\ee
...
As as example, we show how to compute the momentum sum rule
from a PDF set:
\begin{lstlisting}
  call AllocGridQuant(grid,pdf_flav)
  ...
  pdf_flav(:)=sum(pdf_sr(:,-6:6),dim=2)
  moment_index=1_dp ! See definition of truncated moments 
  momentum_sum_rule = GetTruncMoment(grid,pdf_flav,moment_index)
\end{lstlisting}
