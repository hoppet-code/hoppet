...
1.3.0 this is possible and we provide some details in
...
expressed as convolutions of a set of hard coefficient functions and
PDFs, and makes use of the tabulated PDFs and streamlined interface as
...
DIS or VBF, the latter which has already been implemented in the {\tt
  proVBFH}
package~\cite{Cacciari:2015jma,Dreyer:2016oyx,Dreyer:2018qbw,Dreyer:2018rfu}.

An example of how to use the structure functions can be found in
\ttt{example\_f90/structure\_functions\_example.f90}. The structure
functions have been found to be in reasonable agreement with those
which can be obtained with APFEL++. The benchmarks can be found in
...
known\comment{AK: Should we cite some partial results?}.} using both
the exact and parametrised coefficient functions found in
...
In order to set up the structure functions one needs to call 
\begin{lstlisting}
  call StartStrFct(rts, order_max [, nflav] [, xR] [, xF] [, sc_choice] [, cmu]&
                  & [ ,param_coefs] [ ,Qmin_PDF] [ ,wmass] [ ,zmass])
\end{lstlisting}
...
Together \ttt{xR}, \ttt{xF}, \ttt{sc\_choice}, and \ttt{cmu} control
the renormalisation and factorisation scales. The default value for
\ttt{sc\_choice} is $1$ which means that $Q$ multiplied by \ttt{xR} or
\ttt{xF} is used as the renormalisation or factorisation scale
respectively. The choice $0$ corresponds to a fixed scale \ttt{cmu}
(multiplied by \ttt{xR} or \ttt{xF}). Should a user wish to use some
arbitrary scale choice, \ttt{sc\_choice} should be set to $2$. In this
case the structure functions are saved as arrays not only in $Q$ but
also $\mu_R$ and $\mu_F$. This makes for a slightly slower evaluation
compared to the two other scale choices.

If \ttt{param\_coefs} is set to \ttt{.true.}  then the structure
functions are computed using the (very precise) parametrisations found
in
Refs.~\cite{vanNeerven:1999ca,vanNeerven:2000uj,Moch:2004xu,Vermaseren:2005qc,Moch:2008fj,Davies:2016ruz}. Otherwise
the exact versions are used. This however means that the
initialisation becomes very slow, and since the parametrised
expressions are good to $10^{-4}$ for most values of $x$, it is
recommended to use the parametrised option for most applications. Note
that the exact expressions rely on a extended version of
\ttt{hplog}~\cite{FortranPolyLog} that is able to handle harmonic
polylogarithms up to weight 6.\comment{AK: Not sure what to do here
  since the exact expressions return something wrong}.
...
The routine calls \ttt{hoppetStartExtended} and initialises all the
tables needed for the structure functions. In order to fill the
tables, one first needs to set up a \ttt{running\_coupling} object and
evolve the initial PDF with e.g.~\ttt{hoppetEvolve} as described in
sections~\ref{sec:run-coupl} and \ref{sec:vanilla_usage}. With the PDF
table filled one calls
\begin{lstlisting}
...
\begin{lstlisting}
  real(dp) :: xf(-6:7), y, Q, muR, muF
  [...]
  xf = StrFct (y, Q, muR, muF)
\end{lstlisting}
at the value $y=\log{x}$ and $Q$. The variables \ttt{muR} and
\ttt{muF} are not optional, but are only used if \ttt{scale\_choice}
is $2$. The structure functions are in this example stored in the
