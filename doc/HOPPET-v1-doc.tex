...
PDFs, and make use of the tabulated PDFs and streamlined interface as
...
slow, and since the parametrised expressions are good to $10^{-4}$ for
...
handle harmonic polylogarithms up to weight 5.\comment{AK: Not sure
...
also be order by order if the \ttt{separate\_orders} flag was set to
\ttt{.true.} when initialising. This can be done as follows
\begin{lstlisting}
  real(dp) :: xflo(-6:7), xfnlo(-6:7), xfnnlo(-6:7), xfn3lo(-6:7), y, Q, muR, muF
  [...]
  call InitStrFct(4, .true.)
  xflo   = F_LO(y, Q, muR, muF)
  xfnlo  = F_NLO(y, Q, muR, muF)
  xfnnlo = F_NNLO(y, Q, muR, muF)
  xfn3lo = F_N3LO(y, Q, muR, muF)
\end{lstlisting}
...
\ttt{StartStrFct} described above. After evolving or reading in a PDF,
the user then calls
\begin{lstlisting}
  call hoppetInitStrFct(order, separate_orders)
\end{lstlisting}
to initialise the actual structure functions. The structure functions
can then be access through the subroutines
\begin{lstlisting}
  real(dp) :: xflo(-6:7), xfnlo(-6:7), xfnnlo(-6:7), xfn3lo(-6:7), y, Q, muR, muF
  [...]
  call hoppetStrFct(y, Q, muR, muF, xf)         ! Full structure function   
  call hoppetStrFctLO(y, Q, muR, muF, xflo)     ! LO coefficient
  call hoppetStrFctNLO(y, Q, muR, muF, xfnlo)   ! NLO coefficient
  call hoppetStrFctNNLO(y, Q, muR, muF, xfnnlo) ! NNLO coefficient
  call hoppetStrFctN3LO(y, Q, muR, muF, xfn3lo) ! NNLO coefficient
\end{lstlisting}
