...
\end{verbatim}
is similar to that for \ttt{dglap_objects}. The crucial difference is
...
\begin{verbatim}
  ...
  call Multiply(PA,factor)            ! PA = PA * factor                  TEST IT
  call AddWithCoeff(PA,PB[,factor])   ! PA = PA + PB [*factor]

  call SetToConvolution(PA,PB,PC)     ! PA = PB*PC          (opt.alloc)   TEST IT
  call SetToCommutator(PA,PB,PC)      ! PA = PB*PC-PC*PB    (opt.alloc)   TEST IT
...
%
The step size in du can be set with the following call
\begin{verbatim}
...
Lots of name changes will be needed here...
