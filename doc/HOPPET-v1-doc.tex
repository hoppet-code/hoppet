...
in $x-$space.
%Note that 
This technique can be applied to any quantity
...
The piecewise interpolating polynomials that we use are not smooth at
...
\ttt{example\_f77/tabulation\_example.f} (cf.\ Sect.~\ref{sec:stream-example}).
...
discussed in Sect.~\ref{sec:qcd}). For the other options the
