...
\begin{lstlisting}
  call AddNfInfoToPdfTable(table,coupling)
\end{lstlisting}
Finally one creates the tabulation based on the evolution of the PDF from the starting scale $Q_0$ 
\begin{lstlisting}
  call EvolvePdfTableQED(table, Q0, pdf0, dh, qed_split, &
                         coupling, coupling_qed, nloop_qcd, nloop_qed)
\end{lstlisting}
When the argument ${\tt nloop}_{\tt qed}$ is set to 1 the QED corrections of items 1., 2. and 3. listed in Sec.~\ref{sec:qed-evolution} are included,
while if it is set to 0 no QED correction is included. Furthermore, if one sets 
\begin{lstlisting}
with_Plp_nnloqed=.true.
\end{lstlisting}
then also the $P_{lq}$ correction of item 4. is included.\commentgz{Note: at the moment with this flag set to true the $P_{lq}$ contribution is included regardless of the value of $\tt{nloop_qed}$. Perhaps one should change this?} 
