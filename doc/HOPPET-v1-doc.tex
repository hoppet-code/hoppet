...
\gps{Do we need to note that currently StartStrFct itself
  ...
  cleaner, even if it requires an extra line of code from them.} 
%
...
If \ttt{param\_coefs} is set to \ttt{.true.} \gps{(its default)} then the structure
...
slow, and since the parametrised expressions are good to \gps{a relative?}
...
table filled \gps{in the streamlined interface} one calls
\begin{lstlisting}
...
\gps{What is the logic of supplying $y$ rather than $x$? For example,
  hoppetEval takes $x$ --- should we not use the same convention?}
%
