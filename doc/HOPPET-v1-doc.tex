...
\ttt{threshold(i)}.
%
The \ttt{threshold(1:7)} entires are active thresholds, while
...
%
However the \texttt{pdf} array that is passed as an argument and that is
set by those subroutines should range not from \texttt{(-6:6)} but
instead from \texttt{(-6:8)} if the PDF just has photons and
\texttt{(-6:11)} if the PDF also includes leptons.\footnote{Index $7$
  is an artefact associated with internal \hoppet bookkeeping and
  should be ignored in the resulting \ttt{pdf} array.}
...
hadronic structure functions. The structure functions are
...
The structure functions have been found to be in good agreement
with those which can be obtained with APFEL++ (at the level of
...
described in section~\ref{sec:vanilla_usage}.
%
...
and matching studies \cite{DisResum} with Mrinal Dasgupta.
%
