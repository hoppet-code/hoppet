...
version 1.3.0 (and earlier in a dedicated \ttt{qed} branch), is as
described in
...
due to the  \ttt{Plq\_03} splitting function.
% \footnote{
...
Section~\ref{sec:vanilla}.\footnote{Users needing a lower level
  interface should inspect the code in \ttt{src/structure\_functions.f90}.}
%
Then one calls
\begin{lstlisting}
...
\item \ttt{scale\_choice\_Q} (default) means that the code will always
  ...
  and factorisation scale respectively.
...
\item \ttt{scale\_choice\_arbitrary} allows the user to choose
  ...
  $Q$ but also $\mu_R$ and $\mu_F$. \gps{Is this really what is done?}
  % 
