...
(\ttt{m\_light\_quarks}), the three lepton masses (hard-coded to their PDG
values~\cite{ParticleDataGroup:2022pth} in the
\ttt{src/qed\_coupling.f90} file), and the three masses of the heavy quarks
(\ttt{m\_heavy\_quarks(4:6)}).
%
...
\paragraph{PDF arrays with photons and leptons.}
...
\end{lstlisting}
where \ttt{pdf(:,9:11)} contains the sum of lepton and anti-lepton
(which are identical). Note that if one extends the
calculation of lepton PDFs to higher order in ${\cal O}(\alpha)$,
then an asymmetry in the lepton and anti-lepton distribution would arise,
due to the  \ttt{Plq\_03} splitting function.\footnote{
...
\paragraph{PDF tables with photons and leptons.}

Next we must prepair a \ttt{pdf\_table} object forming the
interpolating grid for the evolved PDF's. We remind that the
\ttt{pdf\_table} object contains a table \ttt{pdf\_table\%tab(:,:,:)}
that is a grid in $x$-values (first component), $Q^2$-values (second
component) and PDF values (third component).
...
\paragraph{Evolution with photons and leptons.}
...
\ttt{pdf0} is the PDF at the initial scale. We remind that the upper
scale is stored in the object \ttt{table}.
%
When \ttt{nloopqcd\_qed} is set to 1 (0) mixed QCD-QED effects are
(not) included in the evolution.
%
Setting the variable \ttt{with\_Plq\_nnloqed=.true.}, 
defined in \ttt{module qed\_evolution}, includes also the NNLO $P_{lq}$
...
evolves it to scale \ttt{Q\_end}, overwritting the \ttt{pdf} array. 
%
In order to get interpolated PDF values from the table we use the
\ttt{EvalPdfTable\_*} calls, described in Section~\ref{sec:acc_table}.


Note that at the moment, when QED effects are included, cached evolution is not supported.


\commentgz{Note: at the moment with this flag set to true the $P_{lq}$ contribution is included regardless of the value of $\tt{nloopqcd\_qed}$. Perhaps one should change this?}
...
The \ttt{use\_qed} argument turns on/off QED evolution at order ${\cal O}(\alpha)$
(i.e. items 1. and 2. in Sec.~\ref{sec:qed-evolution}). The \ttt{use\_qcd\_qed} one
turns on/off mixed QCD$\times$QED effects in the evolution (i.e. item
3. in Sec.~\ref{sec:qed-evolution}) and \ttt{use\_Plq\_nnlo} turns on/off the
order $\alpha^2$ $P_{\ell q}$ splitting function (i.e. item 4. in
Sec.~\ref{sec:qed-evolution}). Without this call all QED corrections are off.
