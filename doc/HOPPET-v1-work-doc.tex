...
functionalities of \hoppet, he/she should read in detail
...
used in the LHAPDF PDF library \cite{LHAPDF}.
...
and time-like evolution and splitting functions
 has been investigated in \cite{Dokshitzer:2005bf,Mitov:2006ic,Dokshitzer:2006nm,Beccaria:2007bb}.}, 
...
in Fig.~\ref{fig:interpolation-orders}. 
...
%Note that in all cases $P_{gg}(x)$ is to be
...
\end{lstlisting}
Note that from now on we will drop the explicit use of
$x$ in front of PDF names and convolution operators.
...
\begin{lstlisting}
  ...
  integer           :: nf_lcl ! nf conflicts with global variable
...
Q<28 \TeV$, \\
& \fn~ \ttt{dlnlnQ=dy/4} and the scheme is ${\overline{\rm MS}}$\\
\hline
\begin{lstlisting}
...
scale \ttt{Q0alphas}, \\
 \texttt{\fn~nloop,muR\_Q,LHAsub,Q0pdf)}&
...
\texttt{
 ...
 &
~ ~Initialize a grid derived type \\
\hline
\texttt{
InitGridDef(grid,subgrids,locked=.true.)
}
 &
~ ~Combine subgrids in a single grid \\
\hline
\texttt{
 InitGridQuant(grid,gluon,example\_gluon)
}
 &
~ ~Initialization of a grid quantity \\
\hline
\texttt{
InitGridConv(grid,Pgg,Pgg\_func)
}
 &
~ ~Initialization of a convolution operator \\
\hline
\texttt{InitPDF\_LHAPDF(grid,pdf,LHAsub,Q)}
 &
~ ~Initialization of a PDF grid from LHAPDF-like routine \\
& ~~ Notice it only calls \ttt{LHAsub} at the scale \ttt{Q}\\
\hline
\texttt{
InitDglapHolder(grid, dglap\_h, factscheme, nloop)
}
&
~ ~Initialization of a {\tt dglap\_holder} type \\
\hline
\texttt{
 InitRunningCoupling(coupling [, alfas] [, Q]}
&
~ ~Initialization of a {\tt running\_coupling} type \\
...
SetToConvolution(Pab,Pac,Pcb) 
 ...
 & ~~Convolution of splitting functions \\
\hline
\texttt{Pgg.conv.gluon} & Convolution of a PDF and a splitting function \\
\hline
\texttt{
 EvalPdfTable\_yQ(table, y, Q, pdf)
}
 & ~~Evaluate a tabulated PDF \\
\hline
...
\hline
\texttt{
 EvolvePDF(dglap\_h, initial\_xpdf, coupling,}
 & ~~
...
\hline
\texttt{real(dp), pointer :: gluon(:)} & Grid representation of a 
single PDF \\
\hline
\texttt{real(dp), pointer :: PDFset(:,:)} & 
Grid representation of a PDF set \\
\hline
\texttt{type(grid\_def) :: grid} & Grid in $x-$space\\ 
\hline
\texttt{type(grid\_conv) :: Pgg} 
 & Convolution operator ({\it i.e.} splitting functions) \\ 
\hline
\texttt{ type(split\_mat) :: P}
 & Splitting matrices \\ 
\hline
 \texttt{type(mass\_threshold\_mat) :: MTM\_NNLO}
 & Heavy quark threshold mass matrices\\ 
\hline
\texttt{ type(dglap\_holder) :: dglap\_h} & DGLAP holder \\
\hline
\texttt{type(running\_coupling) :: coupling} & Running coupling \\
\hline
\texttt{type(evln\_operator) :: evop} & Evolution operator \\
\hline
\texttt{type(pdf\_table) :: table} & PDF tabulation \\
\hline
\end{tabular}
\end{center}
\caption{\label{tab:general}
Reference guide for the general interface.
The first table summarizes some of the most relevant methods with
its description, while the second one describes the various
derives types defined in the program. Note that arguments
between \texttt{[...]} are optional. Derived types in
F95 are described in Appendix \ref{sec:f95appendix}.}
\end{table}
...
Finally, there is an option to initialize a multi-flavour
