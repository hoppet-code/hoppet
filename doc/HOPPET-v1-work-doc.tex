...
%%  - document SetCouplingDt
%%  - add 3 new methods for accessing tabulation
%%                       
...
\newcommand{\AllDGLAP}{Botje,Schoeffel:1998tz,Pegasus,Pascaud:2001bi,Weinzierl:2002mv,coriano,GuzziThesis,nnpdf,Kosower:1997hg}
...
%
  ...
  evolution part of the code from f77 and C++.  \smallskip
...
splitting functions to reduce them to discretize so as to obtain fast
...
(\eg~\cite{\AllDGLAP,Ratcliffe:2000kp}), of numerical solutions of the 
DGLAP equation~\cite{DGLAP} for the
...
making the evolution more efficient numerically. Recently,
a novel approach has been proposed which combines
...
\hoppet's ancestors have  been used in a number of different
...
of PDFs in NLO final-state calculations \cite{APPL}, as well
...
it is the author's hope that availability of
...
\label{sec:pqcd}
In this first section we set up the notation and
...
\end{equation}
where the splitting functions are known up to NNLO:
\begin{equation}
  \label{eq:dpdf}
   P(z,\aq)=P^{(0)}(z)+\frac{\aq}{2\pi}P^{(1)}(z)+
\lp \frac{\aq}{2\pi} \rp^2 P^{(2)}(z) \ .
\end{equation}
The generalization to the singlet case is straightforward.
The related variable $t\equiv \ln Q^2/Q_0^2$ is also used
through \hoppet.
...
For light quarks $q_{l,i}$ (quarks which are considered massless
below the heavy quark mass threshold $m_h^2$) the matching reads:
\be
\label{eq:lp-nf1}
...
representation through \hoppet, is such that 
...
same as the one used through the \ttt{LHAPDF} library. 
...
representations of PDFs requires the number of
active quark flavours $n_f$, as described in
...
\hline
\end{tabular}
\caption{}{\label{eq:diag_split} The evolution representation (through
\hoppet called \ttt{evln})
of PDFs in terms of the \ttt{human} representation.}  
\end{center}
\end{table}
...
its convolutions with splitting functions on a grid in $x$,
and the subsequent DGLAP evolution in $Q^2$. 

The first one to be discussed is how to represent PDFs
and associated convolutions in terms of an interpolating grid
in $x-$space.
Note that this technique can be applied to any quantity
which appears through convolutions, so it is not restricted
by any means to parton distributions. Then we will describe how
to
obtain the solution of the DGLAP evolution equations.
...
\end{equation}
where $\beta$ runs over $\mathcal{O}\lp N \rp$ points of the grid and we have
defined
\begin{equation}
  \label{eq:Palphabeta}
...
a practical point of view, we will solve eq.~(\ref{eq:evol})
...
The software package is written in Fortran~95 (F95). This has considerable
...
\texttt{dy}. This grid will use 3rd order interpolation,
the user can modify this choice to better
suit his particular needs. One notes 
...
improve the precision the convolution at large-$x$\footnote{
...
with denser coverage at large-$x$ are required.}. 
To support
this we can first define an array of sub-grids, and then use them to
...
(although several of these uniform grids, each with different
spacing and different coverage in $x$ can be combined simultaneously),
...
\begin{lstlisting}
  function example_gluon(y)
    ...
    x = exp(-y)
    example_gluon = 1.7_dp * x**(-0.1_dp) * (1-x)**5 
  end function example_gluon
\end{lstlisting}
...
from the function \texttt{example\_gluon}.
...
%\footnote{make it more efficient and
...
\begin{lstlisting}
  gluon_at_y_times_2 = 2 * gluon .aty. (y.with.grid)   ! very inefficient
  gluon_at_y_times_2 = 2 * (gluon .aty. (y.with.grid)) ! fairly efficient
  gluon_at_y_times_2 = 2 * EvalGridQuant(grid,gluon,y) ! most efficient
\end{lstlisting}
  ...
  real(dp) function Pgg_func(y)
    ...
    if (cc_piece == cc_DELTA) then   ! Delta function term
       ...
       if (cc_piece == cc_REAL .or. cc_piece == cc_REALVIRT) & 
           & Pgg_func = 2*CA*(x/(one-x) + (one-x)/x + x*(one-x))
...
\end{lstlisting}  
Since the return value of \texttt{Pgg .conv.\ gluon} is just an F95
...
memory leaks as in the previous example.
...
\begin{lstlisting}
  ...
  prep%nf=nf_lcl
  call CopyHumanPdfToEvln(prep, pdf_human, pdf_evln) ! Alternative use
\end{lstlisting}
...
\ttt{evln} representation, or an negative integer if 
the PDF is in the
...
%  answer. This is bad and should be fixed! Trivial modification: add
%  the information in q(:,7).}
...
value of $n_f$. One proceeds in a very similar way than
...
color factors are to be determined from a fit
...
the normalisation is given in eq.~(\ref{eq:dpdf}).
...
Notice that the QCD color factors introduced in Sect. \ref{sec:qcd}
...
present in \ttt{pdf\_nf3} would be ignored.
...
Sect~\ref{sec:dglap-split} and \ref{sec:mtm} respectively. These
...
We are now ready to evolve a multi-flavour PDF. This is done by breaking the
...
likes, for different initial PDFs for example, while for the table, 
...
effectively a different variable each time. It needs to be defined
in a given module and then use that module in each
independent procedure. In Appendix \ref{sec:example_program}, 
...
from C++ --- the interface is essentially identical to the Fortran
...
the number of $x,Q$ points is rather large ($\gtrsim 3000$), 
 it will be $n_{xQ} t_{xQ}$ that takes the most time. We note that
...
like in \cite{de Florian:2007hc}.  
In principle, the
...
resummed. Although studies in this direction
have already been performed, both for small $x$ resummations, 
like in \cite{White:2005wm}
 and for large $x$ resummation, like in \cite{Corcella:2005us},
...
were this GM heavy quark schemes are implemented.
...
the vanilla interface of Sect~\ref{sec:vanilla}, is to be found as
