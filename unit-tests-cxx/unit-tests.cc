...
TEST_CASE( "running_coupling", "[hoppet]" ) {
  ...
  int nf = 5;
  double Qmax = 1e300;
  hoppet::running_coupling alphas(asmz, mz, nloop, nf, Qmax);

  REQUIRE_NOTHROW( alphas(1e200) );
  REQUIRE_THROWS_AS( alphas(Qmax*10), std::runtime_error );
  ...
  alphas = hoppet::running_coupling(asmz, mz, nloop, mc, mb, mt, false, 1.0, Qmax); // variable nf
  REQUIRE_THAT(alphas(mz), WithinAbs(asmz, 1e-6));
  REQUIRE_NOTHROW( alphas(1e200) );
...
TEST_CASE("split-fns", "[hoppet]") {
  ...
  // make sure we are working with a well-defined value of nf
  sl::dh.set_nf(5);
