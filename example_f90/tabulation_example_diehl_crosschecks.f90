...
program tabulation_example_diehl_crosschecks
  ...
  write(6,'(a,i3)') "nloop = ", nloop
  write(6,'(a,i3)') "order = ", order
  write(6,'(a,es14.6)')  "dy = ", dy
  write(6,'(a,es14.6)')  "eps = ", eps
  ...
  if (.not. use_fixed_nf) then
     ...
     write(6,'(a,3f12.5)') "Coupling with dynamic flavours: ", quark_masses
  else
     call InitRunningCoupling(coupling,alfas=0.35_dp,Q=Q0,nloop=nloop,&
          &                   fixnf=4)
     write(6,'(a)') "Coupling with nf=4"
  end if
  ...
  write(6,'(a5,7a16)') "x",&
       & "u-ubar","d-dbar","2(ubr+dbr)","s+sbar","c+cbar","b+bbar", "gluon"
  do ix = 1, size(heralhc_xvals)
     call EvalPdfTable_xQ(table,heralhc_xvals(ix),Q,pdf_at_xQ)
     write(6,'(es7.1,7es16.8)') heralhc_xvals(ix), &
...
end program tabulation_example_diehl_crosschecks
